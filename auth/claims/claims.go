...
// ClaimInvalidError results when an odd error occurs in Standard.Verify.
...
func (e ClaimInvalidError) Error() string {
	switch e.Reason {
	case NotBefore:
		return "claims: standard claim not yet valid"
	case Expired:
		return "claims: standard claim has expired"
	case IssuerMismatch:
		return "claims: standard claim issuer name does not match the expected issuer"
	case IssuedAtFuture:
		return "claims: standard claim issued at a future time"
	case AudienceNotFound:
		return "claims: standard claim audience does not have one of the expected audiences"
	}
